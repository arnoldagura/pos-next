...
export function CheckoutDialog({ open, onOpenChange, locationId }: CheckoutDialogProps) {
  ...
  const isValidPayment = selectedPayment !== 'cash' || tenderedAmount >= (cart?.total || 0);
  ...
  const handleCompleteOrder = async () => {
    ...
    try {
      ...
      setStep('success');
      toast.success(
        orderType === 'dine_in' ? 'Order sent to kitchen!' : 'Order completed successfully!'
      );
    } catch (error) {
            ...
            <div className='flex flex-col items-center justify-center py-8'>
              ...
              {orderNumber && (
                <div className='text-center'>
                  ...
                  <Badge className='text-lg px-4 py-1'>{orderNumber}</Badge>
                </div>
              )}
            </div>
