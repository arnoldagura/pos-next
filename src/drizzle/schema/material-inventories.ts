...
import { pgTable, text, timestamp, numeric, index, unique, pgEnum } from 'drizzle-orm/pg-core';
...
export const materialInventory = pgTable(
  ...
  (table) => ({
    ...
    materialLocationUnique: unique('material_inventory_material_location_unique').on(
      table.materialId,
      table.locationId
    ),
...
export const materialBatch = pgTable(
  ...
  (table) => ({
    ...
    batchNumberIdx: index('material_batch_batch_number_idx').on(table.batchNumber),
...
export const materialInventoryMovement = pgTable(
  ...
  (table) => ({
    materialInventoryIdx: index('material_inventory_movement_material_inventory_idx').on(
      table.materialInventoryId
    ),
    ...
    createdByIdx: index('material_inventory_movement_created_by_idx').on(table.createdBy),
  })
);

export const materialInventoryRelations = relations(materialInventory, ({ one, many }) => ({
...
export const materialBatchRelations = relations(materialBatch, ({ one, many }) => ({
